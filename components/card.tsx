...
import { AlertDialogPokedex } from "./alert-dialog";
...
export default function PokemonCard({ item, color }: Props) {
  ...
  const handleDropp = (item: results) => {
    const savedPokedex = localStorage.getItem("pokedex")
  ...
  return (
    <div className="relative transform transition-transform duration-300 hover:scale-105">
      <div
        className={` md:w-[150px] max-sm:w-[75px] shadow-xl p-3 ${colorClass} rounded-md`}
      >
        <Link href={`/list-pokemon/${item.name}`}>
          <img src={item.image} alt={item.name} className="w-full h-30" />
        </Link>
        <div className="flex justify-center">
          <div className="font-bold md:text-center max-sm:text-[12px]">
        ...
        <div className="flex justify-center">
          {path === "/list-pokemon/pokedex" ? (
            ...
            <AlertDialogPokedex title="Successfully added to pokedex">
              <button
